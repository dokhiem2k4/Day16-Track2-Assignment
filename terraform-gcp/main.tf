...
resource "google_compute_instance" "gpu_node" {
  ...
  boot_disk {
    initialize_params {
      # Standard Debian 11 image (no CUDA needed for CPU workload)
  ...
  # guest_accelerator {   # Disabled: CPU fallback plan (no GPU quota)
  #   type  = var.gpu_type
  #   count = var.gpu_count
  # }

  scheduling {
    on_host_maintenance = "MIGRATE"  # Required when no GPU attached
